...
SIGNAL PRICE 				: INTEGER RANGE 0 TO 999;
SIGNAL TENDER				: INTEGER RANGE 0 TO 999 := 0;
SIGNAL CHANGE				: INTEGER RANGE 0 TO 999;
...
--Keep track of previous inputs 111 and 11 represent no input
SIGNAL PREV_ROW : STD_LOGIC_VECTOR(2 downto 0) := "111";
SIGNAL PREV_COL : STD_LOGIC_VECTOR(1 downto 0) := "11";
SIGNAL PREV_TENDER: INTEGER RANGE 0 TO 999 := 111;
...
FUNCTION convertToHex(digit : INTEGER) RETURN STD_LOGIC_VECTOR IS
		BEGIN
		CASE digit IS
		  WHEN 0 => RETURN HEX_ZERO;
		  WHEN 1 => RETURN HEX_ONE;
		  WHEN 2 => RETURN HEX_TWO;
		  WHEN 3 => RETURN HEX_THREE;
		  WHEN 4 => RETURN HEX_FOUR;
		  WHEN 5 => RETURN HEX_FIVE;
		  WHEN 6 => RETURN HEX_SIX;
		  WHEN 7 => RETURN HEX_SEVEN;
		  WHEN 8 => RETURN HEX_EIGHT;
		  WHEN 9 => RETURN HEX_NINE;
		  WHEN OTHERS => RETURN HEX_OFF;
		END CASE;
END FUNCTION;
		...
		ELSIF (Rising_Edge(Clock)) THEN
			CASE STATE IS
				WHEN STATE0 =>
					...
					CASE Col IS
						WHEN "00" => HEX_COL <= HEX_ZERO;
						WHEN "01" => HEX_COL <= HEX_ONE;
						WHEN "10" => HEX_COL <= HEX_TWO;
						WHEN "11" => HEX_COL <= HEX_THREE;
						WHEN OTHERS => HEX_COL <= HEX_OFF;
					END CASE;

					CASE Row IS
						WHEN "000" => HEX_ROW <= HEX_A;
						WHEN "001" => HEX_ROW <= HEX_B;
						WHEN "010" => HEX_ROW <= HEX_C;
						WHEN "011" => HEX_ROW <= HEX_D;
						WHEN "100" => HEX_ROW <= HEX_E;
						WHEN "101" => HEX_ROW <= HEX_F;
						WHEN "110" => HEX_ROW <= HEX_G;
						WHEN OTHERS => HEX_ROW <= HEX_OFF;
					END CASE;
					...
					HEX_COUNTER1 <= convertToHex(PRICE_ONES);
					HEX_COUNTER2 <= convertToHex(PRICE_TENTHS);
					HEX_COUNTER3 <= convertToHex(PRICE_HUNDREDTHS);
					
				WHEN STATE1 => 
					IF (Rising_Edge(Adjust_Up)) THEN
						TENDER <= TENDER + 5;
						...
						--convert each digit to hex and send to 7 segment display
						HEX_COUNTER1 <= convertToHex(TENDER_ONES);
						HEX_COUNTER2 <= convertToHex(TENDER_TENTHS);
						HEX_COUNTER3 <= convertToHex(TENDER_HUNDREDTHS);
						
					ELSIF (Rising_Edge(Adjust_Down)) THEN
						TENDER <= TENDER - 5;
						--grab each digit of TENDER
						TENDER_ONES <= TENDER / 100;
						TENDER_TENTHS <= (TENDER MOD 100) / 10;
